...
import yaml
from pannuke_ssl.ssl_framework import apply_lr, load_standard_config, run_downstream, run_tuning, train_ssl, write_final_report
...
def main():
    ...
    elif a.action=="report": result={"report":str(write_final_report(c))}
    else:
        tune=run_tuning(c); c=apply_lr(c,float(tune["selected_value"])); pre=train_ssl(c,root/"pretrain"); down=run_downstream(c,root/"pretrain/checkpoints/best.pt",root/"downstream"); report=write_final_report(c); result={"tuning":tune,"pretrain":pre,"downstream":down,"report":str(report)}
