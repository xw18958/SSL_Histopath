"""Isolated, audited final probe for the selected I-JEPA duration-pilot encoder."""
...
ENCODER = ROOT / "outputs/ijepa_duration_pilot/checkpoints/best.pt"
OUT = ROOT / "outputs/ijepa_duration_pilot_final_probe"
...
def old_manifest():
    return protected_manifest(ROOT)
...
def verify_lock(c):
    ...
    assert old_manifest() == lock["protected_b0_manifest"]
...
def lock_encoder(c):
    ...
    duration = json.loads((ROOT / "outputs/ijepa_duration_pilot/duration_selection.json").read_text())
    assert checkpoint["epoch"] == duration["selected_epoch"]
    assert sha(ENCODER) == duration["checkpoint_sha256"]
    assert old_manifest() == json.loads((ROOT / "outputs/ijepa_duration_pilot/protected_manifest.json").read_text())
    assert checkpoint["config"]["train"]["epochs"] == 300
    assert checkpoint["config"]["train"]["warmup_fraction"] == 0.1
    assert checkpoint["config"]["output_dir"] == str(ROOT / "outputs/ijepa_duration_pilot")
    ...
    lock = {"checkpoint_path": str(ENCODER), "checkpoint_sha256": sha(ENCODER),
            ...
            "metadata_sha256": sha(c["metadata_csv"]), "protected_b0_manifest": old_manifest(),
...
def test_once(c):
    ...
    ax.set_title("I-JEPA duration-pilot encoder: single-seed transductive linear probe")
    for i in range(19):
        for j in range(19):
    ...
    plt.close(fig)
    summary = {"evaluation_protocol": "single-seed, transductive I-JEPA linear-probe result",
...
def main(stage):
    ...
    p.add_argument("--config", default=str(ROOT / "configs/ijepa_duration_final_probe.yaml"))
    args = p.parse_args()
