"""Isolated, audited final probe for the selected B1 duration-pilot encoder.

Only the ``test`` stage can construct a test loader.  It atomically marks the
output directory before that construction and cannot be rerun after a crash.
"""
...
from pannuke_ssl.b1_training import load_protected_manifest, verify_protected_manifest
from pannuke_ssl.utils import atomic_json_dump, seed_everything, write_csv

ROOT = Path("/raid1/xwan0900/SSL_proj")
PILOT_OUT = ROOT / "outputs/b1_duration_pilot"
ENCODER = PILOT_OUT / "checkpoints/best.pt"
OUT = ROOT / "outputs/b1_duration_pilot_final_probe"
...
def protected_reference_manifest():
    verify_protected_manifest(PILOT_OUT)
    return load_protected_manifest(PILOT_OUT)
...
def verify_lock(c):
    ...
    assert protected_reference_manifest() == lock["protected_b0_ijepa_manifest"]
...
def lock_encoder(c):
    ...
    duration = json.loads((PILOT_OUT / "duration_selection.json").read_text())
    assert checkpoint["epoch"] == duration["selected_epoch"]
    assert sha(ENCODER) == duration["checkpoint_sha256"]
    assert protected_reference_manifest() == json.loads((PILOT_OUT / "protected_manifest.json").read_text())
    assert checkpoint["config"]["train"]["epochs"] == 300
    assert checkpoint["config"]["train"]["warmup_fraction"] == 0.1
    assert checkpoint["config"]["output_dir"] == str(PILOT_OUT)
    ...
    lock = {"checkpoint_path": str(ENCODER), "checkpoint_sha256": sha(ENCODER),
            "student_state_sha256": tensor_sha(state), "epoch": checkpoint["epoch"],
            "schedule_epochs": 300, "warmup_epochs": 30,
            "wording": f'epoch {checkpoint["epoch"]} of the 300-epoch schedule with 30-epoch warmup',
            "metadata_sha256": sha(c["metadata_csv"]), "protected_b0_ijepa_manifest": protected_reference_manifest(),
            "torch_version": torch.__version__, "cuda_version": torch.version.cuda,
            "gpu": torch.cuda.get_device_name(0), "ssl_seed": checkpoint["config"]["seed"]}
    metadata(c)
    # All checks occur before exclusive creation, so a bad selected checkpoint
    # cannot leave a partial final-probe directory behind.
...
def extract(c, splits, lock):
    ...
    # Do not validate or materialize non-requested splits.  In particular,
    # train/validation extraction never touches a test image.
...
def test_once(c):
    ...
    with (OUT / "test_started.json").open("x") as f:
        json.dump({"selection_sha256": sha(OUT / "selection.json"),
                   "probe_checkpoint_sha256": selection["probe_checkpoint_sha256"],
                   "encoder_checkpoint_sha256": lock["checkpoint_sha256"], "time": time.time()}, f)
    ...
    ax.set_title("B1 residual discrete-velocity encoder: single-seed transductive linear probe")
    for i in range(19):
        for j in range(19):
    ...
    plt.close(fig)
    summary = {"evaluation_protocol": "single-seed, transductive B1 residual discrete-velocity linear-probe result",
               ...
               "test_marker_sha256": sha(OUT / "test_started.json"),
               "selection_sha256": sha(OUT / "selection.json"),
               "predictions_sha256": sha(OUT / "test_predictions.npz"),
...
def main(stage):
    ...
    p.add_argument("--config", default=str(ROOT / "configs/b1_duration_final_probe.yaml"))
    args = p.parse_args()
